...
async function loadCommission(): Promise<CommissionConfig> {
  const snap = await db.collection("config").doc("platform").get();
  const stored = (snap.data()?.["commission"] ?? {}) as Partial<CommissionConfig>;
  return { ...DEFAULT_COMMISSION, ...stored };
}

async function isSuperAdmin(uid: string): Promise<boolean> {
  const user = (await db.collection("users").doc(uid).get()).data() as Doc | undefined;
...
export const setCommissionConfig = onCall(async (req) => {
  ...
  if (!(await isSuperAdmin(uid))) throw new HttpsError("permission-denied", "Not allowed.");
  const input = (req.data ?? {}) as Partial<CommissionConfig>;
...
async function audit(event: string, data: Doc): Promise<void> {
  try {
...
async function nextSequence(name: string): Promise<number> {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const value = ((snap.data()?.["value"] as number | undefined) ?? 4820) + 1;
 ...
 */
async function markPaidAndIssueReceipt(
  orderId: string,
  payment: { cfPaymentId?: string | null; cfOrderId?: string | null },
): Promise<string> {
  ...
  const { receiptId, issued } = await db.runTransaction(async (tx) => {
    ...
    if (existing) return { receiptId: existing, issued: false };
...
export const createCheckoutOrder = onCall({ secrets: CASHFREE_SECRETS }, async (req) => {
  ...
  const lines = (Array.isArray(req.data?.items) ? req.data.items : []) as CartLineInput[];
  ...
  if (existingDoc) {
    const existing = existingDoc.data() as Doc;
  ...
  const shop = shopSnap.data() as Doc | undefined;
  ...
  if (shop["status"] !== "OPEN") {
    throw new HttpsError(
      "failed-precondition",
      "Online ordering is temporarily unavailable for this shop.",
    );
  }
  const vendorId = shop["vendorId"] as string | undefined;
  ...
  for (const line of lines) {
    ...
    const itemSnap = await db
      ...
      .get();
    const item = itemSnap.data() as Doc | undefined;
...
function paymentMatches(order: Doc, amount: number, currency: string): boolean {
  return (
    Math.abs(round2(amount) - round2(Number(order["totalAmount"]))) < 0.01 &&
...
export const verifyCashfreePayment = onCall({ secrets: CASHFREE_SECRETS }, async (req) => {
  ...
  if (order["receiptId"]) {
    return { status: "SUCCESS", receiptId: order["receiptId"] as string };
  }
  if (!cashfreeConfigured()) throw new HttpsError("failed-precondition", "Payment could not be started.");
  ...
  if (status === "ACTIVE") {
    ...
    const payments = await fetchOrderPayments(orderId);
    const pending = payments.some((p) =>
      ["PENDING", "USER_DROPPED", "NOT_ATTEMPTED"].includes(String(p.payment_status).toUpperCase()),
    );
...
export const cashfreeWebhook = onRequest({ secrets: WEBHOOK_SECRETS }, async (req, res) => {
  ...
  const data = (payload["data"] ?? {}) as Doc;
  ...
  const eventId = `${type}:${orderId}:${String(
    data["payment"]?.["cf_payment_id"] ?? data["refund"]?.["refund_id"] ?? timestamp,
  )}`;
  ...
  try {
    ...
    const order = orderSnap.data() as Doc | undefined;

    if (type === "PAYMENT_SUCCESS_WEBHOOK" && order) {
      const payment = (data["payment"] ?? {}) as Doc;
      ...
      const refund = (data["refund"] ?? {}) as Doc;
      ...
      const settlement = (data["settlement"] ?? data["order"] ?? {}) as Doc;
      ...
      if (order) {
        ...
        await db
          ...
          .update({
            ...
            shopNetAmount: round2(
              Number(order["shopAmount"] ?? 0) - (borneByShop ? charges : 0),
            ),
            updatedAt: now(),
          });
      }
...
export const redeemReceipt = onCall(async (req) => {
  ...
  if (!receiptId && receiptNo) {
    ...
    if (!first) throw new HttpsError("not-found", "Receipt not found.");
    ref = first.ref;
  }
  ...
  const redeemedAt = await db.runTransaction(async (tx) => {
    ...
    const receipt = snap.data() as Doc;
...
export const connectShopPayouts = onCall({ secrets: CASHFREE_SECRETS }, async (req) => {
  ...
  const vendorId = (shop["vendorId"] as string | undefined) ?? `shop_${shopId}`;
...
export const refundOrder = onCall({ secrets: CASHFREE_SECRETS }, async (req) => {
  ...
  const shop = (await db.collection("shops").doc(order["shopId"]).get()).data() as Doc | undefined;
  const allowed = shop?.["ownerId"] === uid || (await isSuperAdmin(uid));
  if (!allowed) throw new HttpsError("permission-denied", "Not allowed.");
  if (!cashfreeConfigured()) throw new HttpsError("failed-precondition", "Payments are not configured.");
  ...
  if (amount <= 0) throw new HttpsError("failed-precondition", "Nothing left to refund.");
...
export const platformFinancials = onCall(async (req) => {
  ...
  snap.forEach((d) => {
    ...
    if (t["paymentStatus"] === "PAYMENT_SUCCESS") totals.successfulPayments += 1;
    if (t["settlementStatus"] === "SETTLED") totals.settled += Number(t["shopAmount"] ?? 0);
    else totals.pendingSettlement += Number(t["shopAmount"] ?? 0);
  });
